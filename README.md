...
```
...and similarly creating a `CsDate` object from the Julian Day Number:
```
>>> from pythaidate import CsDate, julianday
...
384
```
Convert to a `datetime.date` object with the `julianday.julianday_to_date` helper function:
```
>>> julianday.julianday_to_date(cs.julianday)
